    ...
    run: async (client, message, args) => {
        if (args[0] == "close") {
            if (user_tickets.has(message.channel.id)) {
                ...
                let user2 = message.guild.members.cache.get(`${user}`)

                if (user2.roles.cache.has("828319282068324382") || user2.roles.cache.has("826924370210979851")) {
            ...
            if (!message.member.roles.cache.has("828319282068324382")) return

            if (!args[1]) return message.channel.send("Please provide a User ID.")

            if (user_tickets.has(`${args[1]}_blacklisted`)) {
                return message.channel.send("That user is already blacklisted.")
            } else {
                user_tickets.set(`${args[1]}_blacklisted`, true)

                message.channel.send("The user has been blacklisted.")
            }
            ...
            if (!user_tickets.has(`${args[1]}_blacklisted`)) {
                return message.channel.send("That user isn't blacklisted.")
            } else {
                user_tickets.delete(`${args[1]}_blacklisted`)
